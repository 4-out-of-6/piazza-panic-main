package Sprites;

import Ingredients.Potato;
import Ingredients.Ingredient;
import com.badlogic.gdx.maps.tiled.TiledMap;
import com.badlogic.gdx.math.Rectangle;
import com.badlogic.gdx.physics.box2d.BodyDef;
import com.badlogic.gdx.physics.box2d.World;

/**
 * The `CheeseStation` class extends the `InteractiveTileObject` class.
 * It is used to represent a station for producing lettuce in the game.
 */

public class PotatoStation extends InteractiveTileObject {

    /**
     * Constructs a CheeseStation.
     *
     * @param world the world where the CheeseStation will be located
     * @param map the TiledMap that the CheeseStation is in
     * @param bdef the BodyDef for the physics of the CheeseStation
     * @param rectangle the rectangle representing the boundaries of the CheeseStation
     */

    public PotatoStation(World world, TiledMap map, BodyDef bdef, Rectangle rectangle) {
        super(world, map, bdef, rectangle);
        fixture.setUserData(this);
    }

    /**
     * Returns the ingredient Cheese.
     *
     * @return a Cheese object
     */
    public Ingredient getIngredient(){
        return (new Potato(0,3));
    }
}


